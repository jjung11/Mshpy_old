...
if argv[1]=='cs': run='Msh_Nstep_cs_002'
else: run='Msh_Nstep_002'

x=np.linspace(0,20)

for i in range(1,7):
    a=pd.read_csv('/Volumes/Elements/openggcm_run/'+run+'/modeling/Msh_data_0.1.'+str(1200+1200*i).zfill(6),header=None,sep='\s+',names=['the','phi','m','rad','x','y','z','bx1','by1','bz1','vx1','vy1','vz1','rr1','pp1','ex1','ey1','ez1','xjx1','xjy1','xjz1','resis1']).drop_duplicates()
    a2=a[a.the==0]
    if argv[1]=='cs':
        b=pd.read_csv('/Volumes/Elements/openggcm_run/'+run+'/sheath_data_ascii/cs%d.txt' % i,sep='\s+',header=None,names=['theta_1','theta_2','f_1','f_2','B','ni','Ti','V'],skiprows=1)
    else:
        b=pd.read_csv('/Volumes/Elements/openggcm_run/'+run+'/sheath_data_ascii/cn%d.txt' % i,sep='\s+',header=None,names=['theta_1','theta_2','f_1','f_2','B','ni','Ti','V'],skiprows=1)
...
#    b2=b[(-90<=b.phi) & (b.phi<=90)]
    ...
    buf=mhd_plot_main([':/'+run+'.3df.'+str(1200+1200*i).zfill(6)+':/Volumes/Elements/openggcm_run/'+run+':.',
    ...
    for phi in np.arange(-90+7.5,90,7.5):
        ...
        comp2.to_csv('/Volumes/Elements/openggcm_run/'+run+'/modeling/comp_phi_0.1_%.1f_' % phi+str(1200+1200*i).zfill(6),float_format='%.3f')
        ...
        fig.savefig('/Volumes/Elements/openggcm_run/'+run+'/modeling/plots/comp/comp_%d_phi_0.1_%.1f.png' % (20*i+20,phi))
        plt.close()
