...
if lf!=1:stat.to_csv('/Volumes/'+external+'/openggcm_run/'+run+'/modeling/stat_0.1.txt')
else:stat.to_csv('/Volumes/'+external+'/openggcm_run/'+run+'/modeling/stat_lf_0.1.txt')
...
for j in range(4):
    ...
    r2.append(r2_score(a3[list[j]+'_T'][mask],a3[list[j]][mask]))
...
#    ax_t.annotate('y={:.2f}x+{:.2f}'.format(reg[j].slope,reg[j].intercept),xy=(.05,.95),xycoords='axes fraction')
    ax_t.text(0.05,0.9,'R$^2=${:.2f}'.format(r2[j]),transform=ax_t.transAxes,fontsize=15,bbox=dict(facecolor='white'))
...
if lf!=1:plt.savefig('/Volumes/'+external+'/openggcm_run/'+run+'/modeling/plots/comp/Scatter_0.1.png')
else:plt.savefig('/Volumes/'+external+'/openggcm_run/'+run+'/modeling/plots/comp/Scatter_lf_0.1.png')
