...
#  print(Dmhd)
  ...
  fMPBS = Dmhd+'/modeling/'+info.run+'.MPBS'+pn+'_car_j.'+info.mhdt
  #print('fmhd',fmhd)
...
for i in range(1,2):
    a=pd.read_csv('/Volumes/Elements/openggcm_run/Msh_Nstep_cs/modeling/Msh_data.'+str(1200+1200*i).zfill(6),header=None,sep='\s+',names=['the','phi','m','rad','x','y','z','bx1','by1','bz1','vx1','vy1','vz1','rr1','pp1','ex1','ey1','ez1','xjx1','xjy1','xjz1','resis1']).drop_duplicates()
    a2=a[a.the==0]
    b=pd.read_csv('/Volumes/Elements/openggcm_run/Msh_Nstep_cs/sheath_data_ascii/cs%d.txt' % i,sep='\s+',header=None,names=['theta_1','theta_2','f_1','f_2','B','ni','Ti','V'],skiprows=1)
...
#    b2=b[(-90<=b.phi) & (b.phi<=90)]
    ...
    buf=mhd_plot_main([':/Msh_Nstep_cs.3df.'+str(1200+1200*i).zfill(6)+':/Volumes/Elements/openggcm_run/Msh_Nstep_cs:.',
    ...
    for phi in np.arange(-90+7.5,90,7.5):
        ...
        for j in range(1,10):
            ...
            a_t2=a_t[(a_t.m>10*j-10) & (a_t.m<10*j+10)]
        ...
        comp2.to_csv('/Volumes/Elements/openggcm_run/Msh_Nstep_cs/modeling/comp_phi_%.1f_' % phi+str(1200+1200*i).zfill(6),float_format='%.3f')
        ...
        fig.savefig('/Volumes/Elements/openggcm_run/Msh_Nstep_cs/modeling/plots/comp/comp_%d_phi_%.1f.png' % (20*i+20,phi))
        plt.close()
